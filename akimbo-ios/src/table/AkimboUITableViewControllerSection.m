...
@interface AkimboUITableViewControllerSection ()
@end

@implementation AkimboUITableViewControllerSection
...
- (id)initWithStore:(AkimboDataStore *)newStore
{
    self = [self init];
    if (self) {
        self.store = newStore;
...
- (void)initComponent
{
    ...
    self.store = [[AkimboDataStore alloc] init];
    customView = nil;
    canEdit = NO;
    canDelete = NO;
    cellHeight = 44.0f;
...
//  Mutators
//////////////////////////////////////////////////////////////////////////////////////////////
- (void)setTableViewController:(AkimboUITableViewController *)newTableViewController
{
...
- (void)setStore:(AkimboDataStore *)dataStore
{
...
- (void)renderCell:(UITableViewCell *)cell withRecord:(id)record atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([cell isKindOfClass:[AkimboUITableViewCell class]] && [cell respondsToSelector:@selector(setSection:)] && [cell respondsToSelector:@selector(setRecord:)]) {
...
{
    [self selectedCell:cell withRecord:record andCallDelegate:YES];
}

- (void)selectedCell:(UITableViewCell *)cell withRecord:(id)record andCallDelegate:(BOOL)callDelegate;
{
    //Get the index path
...
{
    [self selectRecord:record callDelegate:YES];
}
...
- (void)selectRecord:(id)record callDelegate:(BOOL)callDelegate
{
    ...
    //Call the select cell method
    [self selectedCell:cell withRecord:record andCallDelegate:callDelegate];
