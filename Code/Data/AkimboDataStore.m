...
@implementation AkimboDataStore
...
- (void)filterByFieldName:(NSString *)fieldName equalTo:(id)value
{
    [self addFilter:[NSPredicate predicateWithFormat:@"%K == %@", fieldName, value]];
}

- (void)filterByFieldName:(NSString *)fieldName notEqualTo:(id)value
{
    [self addFilter:[NSPredicate predicateWithFormat:@"%K != %@", fieldName, value]];
}
...
- (void)filterByFieldNameIsTrue:(NSString *)fieldName
{
    [self addFilter:[NSPredicate predicateWithFormat:@"%K == YES", fieldName]];
}

- (void)filterByFieldNameIsFalse:(NSString *)fieldName
{
    [self addFilter:[NSPredicate predicateWithFormat:@"%K == NO", fieldName]];
}
...
- (void)removeAll
{
    ...
    [self onRemoveAll:removedItems.copy];
}
...
- (NSArray *)uniqueValuesForFieldName:(NSString *)fieldName
{
    ...
    @synchronized(allItems){
        return [allItems valueForKeyPath:keyPath];
    }
}
...
- (void)onRemoveAll:(NSArray *)removedItems
{
    ...
    if (delegate != nil && [delegate respondsToSelector:@selector(dataStore:removeAll:)]) {
        [delegate dataStore:self removeAll:removedItems];
    }
    
    [self callDelegatesWithSelector:@selector(dataStore:removeAll:) andArguments:@[self, removedItems]];
...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if (self) {
        ...
        @synchronized(self.allItems) {
            @synchronized(aDecoder) {
                decodedItems = [aDecoder decodeObjectForKey:@"allItems"];
            }
        }
        [self load:decodedItems];
    }
	return self;
}
